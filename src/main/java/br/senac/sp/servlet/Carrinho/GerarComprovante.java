...
import br.senac.sp.dao.CardDAO;
...
public class GerarComprovante extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (formaPagamento.getPayment_way().equals("Boleto")) {
                codPagamento = PaymentDAO.addPaymentWithoutCard(formaPagamento);
            } else {  
            Card card = (Card) sessao.getAttribute("cartaoPag");
            int idCartao = CardDAO.addCard(card);
                System.out.println("OLHA AQUI O ID DO CARTAO");
            Payment PaymentByCard = new Payment(formaPagamento.getPayment_way(), formaPagamento.getPayment_instalments(), formaPagamento.getPayment_status(), idCartao);
            
            
            codPagamento = PaymentDAO.addPaymentWithCard(PaymentByCard);
            }
            codPagamento = PaymentDAO.addPaymentWithoutCard(formaPagamento);
            ...
            Order order = new Order(codPedido.toString(), vlrTotal, usuario.getCustomer_id(), codPagamento, "Aguardando pagamento", addr.getAddress_id());
